  ...
  <script language="JavaScript" type="text/javascript">
    $(function () {
     /* $.post("getTypeData", null, function (data) {
      ...
      $.post("getTypeData",null,function(data){
        //加载类型
        //[{"id":1000,"name":"一室一厅"},{"id":1001,"name":"一室两厅"},{"id":1005,"name":"三室两厅"},{"id":1006,"name":"四室一厅"},{"id":1007,"name":"四室两厅"}]
        for(var i=0;i<data.length;i++){
          //创建option
          var optionnode=$("<option value="+data[i].id+">"+data[i].name+"</option>");
          //将option添加到下拉列表
...
name=floorage></TD></TR>
  ...
  <TR>
    ...
    value="<f:formatDate value="${house.pubdate}"
pattern="yyyy-MM-dd"></f:formatDate>"class=text type="data"name=pubdate> </TD></TR>
  <TR>
    ...
    <TD>区：
      ...
      街：<SELECT class=text name=street_id id="street_id">
        <option value="">请选择</option>
      </SELECT> </TD></TR><!--
						<tr>
  ...
  <TR>
    ...
    <TD><img src="http://localhost:80/${house.path}"><input type="file" name="pfile"></TD>
  </TR>
  </TBODY></TABLE>
<DIV class=buttons><INPUT typeof="submit" value=更新发布 type=button>
